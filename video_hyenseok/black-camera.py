...
out = cv2.VideoWriter('black.avi', fourcc, 20.0, (640,480)) #output.avi로 저장됨, 코덱, fps, 프레임크기
...
while True:
    ...
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) #흑백영상으로 전환.
    out.write(frame) #영상 저장 
    cv2.imshow("gray", gray) #흑백 영상 출력 원래영상,
