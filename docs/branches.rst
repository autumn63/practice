...
- 기대 효과:
  ...
  - 욕설·혐오 표현 사전 제거로 데이터 정제 및 안전성 확보
...
- 브랜치 이름: ``image``
- 역할: 이미지(JPG/PNG 등) 불러오기 → 블러 처리 → 반전 → 드래그 기반 크롭을 수행하여
  후속 작업에 적합한 정제된 이미지로 변환한다.

- 주요 기능 흐름:

  - ``Input``: 원본 이미지 파일 입력
  - ``Load``: Pillow 기반 이미지 로드 (``load_image.py``)
  - ``Transform``: Blur, Flip(좌우/상하), Drag Crop 수행
  - ``Crop``: 드래그한 영역을 그대로 잘라내는 ROI 기반 크롭
  - ``Output``: 처리 결과 저장 및 파일명 규칙 기반 버전 관리

- 모듈 구성:

  - ``blur.py`` — 블러 처리 모듈
    - ``GaussianBlur`` 기반 흐림 효과 적용
    - 이미지 크기를 유지한 채 소프트닝 처리
  - ``flip_horizontal.py`` / ``flip_vertical.py`` — 반전 엔진
    - 좌우 / 상하 반전 기능 제공
  - ``crop.py`` — 크롭 엔진
    - 드래그 시작/종료 좌표 기반 사각형 ROI 계산
    - 선택 영역 그대로 잘라낸 크롭 이미지 반환
  - ``load_image.py`` / ``save_image.py`` — I/O 유틸리티
    - 이미지 로드 및 저장 처리
    - 출력 폴더 자동 생성 및 구조화된 파일명 관리
  - ``main.py`` — 파이프라인 컨트롤러
    - 입력 이미지 선택
    - 지정된 전처리 기능 실행(blur/flip/crop)
    - 예외 처리 및 흐름 제어
  - ``test_image_funcs.py`` — 테스트 스위트
    - 블러 이후 크기 유지 여부 테스트
    - 반전/크롭 기능 검증을 위한 단위 테스트 포함

- 사용 기술 스택:

  - ``Language``: Python
  - ``Libraries``: Pillow(PIL), ``os``, ``unittest``
  - ``Data``: 사용자 입력 이미지 파일

- 기대 효과:

  - 드래그 기반 크롭을 통해 관심 영역만 빠르게 분리 가능
  - 블러·반전·크롭 기능을 모듈화하여 재사용성과 확장성 확보

- 오류 발생:
  - 이 오류는 macOS와 Anaconda에 포함된 Tkinter(GUI 라이브러리) 버전 간의 충돌로 인해 발생하는 잘 알려진 버그입니다.
   
  - 특히 파일 열기/저장 창(filedialog)을 띄울 때 filetypes 옵션(확장자 필터)을 처리하는 과정에서 macOS의 네이티브 파일 탐색기와 호환되지 않아 앱이 강제 종료되는 현상입니다.
...
- 주요 프로세스:
  ...
  - ``Input``: 비디오 파일 입력 (``data_video/video.mp4``)
...
- 모듈 구성:
  ...
  - ``main2.py`` (Video Renderer)
    - 저장된 프레임 자동 로드
    - FPS 기반 영상 재구성
    - XVID 코덱 기반 MP4 영상 생성
    - 최종 출력 파일(``data_video/final_output.mp4``) 저장
...
- 주요 프로세스:
  ...
  - ``Input``: 비디오 파일 경로를 지정하여 ``cv2.VideoCapture`` 로 로드  
    (입력 영상: ``video_yeonbeom/video.mp4``)
  - ``Inspect``: 첫 프레임을 읽어 원본 해상도(``width``, ``height``)와 FPS 정보를 출력
  - ``Writer``: 원본 영상의 FPS·해상도에 맞춰 ``cv2.VideoWriter`` 로
    출력 파일(``video_hyenseok/output_blur.mp4``) 생성
  ...
  - ``Display``: 보기 편하도록 가로 1280px 기준으로 리사이즈하여 출력
...
- 스크립트 구성 (``main3.py``):
  ...
  - 얼굴 감지 루프
    - ``with mp_face_detection.FaceDetection(model_selection=1,
      min_detection_confidence=0.1)`` 블록 안에서 매 프레임 처리
